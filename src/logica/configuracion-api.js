(function configurarClienteApi() {
  ...
  const CLAVE_LOCAL_STORAGE = "ECOMMERCE_API_BASE_URL";
  ...
  function obtenerBaseUrlDesdeStorage() {
    try {
      return normalizarBaseUrl(window.localStorage.getItem(CLAVE_LOCAL_STORAGE));
    } catch (error) {
      return "";
    }
  }
  ...
  function resolverBaseUrlBackend() {
    ...
    const baseUrlGuardada = obtenerBaseUrlDesdeStorage();
    if (baseUrlGuardada) return baseUrlGuardada;
